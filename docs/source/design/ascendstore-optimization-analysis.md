...
| 序号 | 需求 | 子项 | 优先级 | 优先级理由 | 工作量（人月） | 建议人数 | 周期 |
|------|------|------|--------|-----------|-------------|---------|------|
| 1 | Layerwise 池化传输与 DSA KV Offload 接口 | 1A~1F（6 个子接口） | **高** | vLLM-Ascend 直接诉求；上游已有 session API（PR #2881）+ allocate_host_buffer（PR #3019）基础，增量投入即可产出；是存算分离（需求 3）的前置依赖 | 2.20 | 3 人 | 10 周 |
| 2 | MooncakeStore 高可用加固 | 2A HA 基于 K8s | **高** | 商用必须；当前 HA 仅 etcd Active-Standby，K8s 原生 HA PR 已提交（#1856/#1910/#1956），合入后即可适配；阻塞商用部署 | ~1.5 | 1.5 人 | Q3~Q4 |
| 2 | MooncakeStore 高可用加固 | 2B Master 切换元数据恢复 | **高** | 商用必须；主备切换数据丢失是致命缺陷；RFC #1150 已有方案但未闭环，需推动实现；涉及 Master 架构演进 | ? | ? | Q4~Q1 |
| 2 | MooncakeStore 高可用加固 | 2C HIXL RAS core dump | **高** | RAS 场景必过项；Issue #2440 core dump 阻塞故障注入测试；影响生产稳定性 | 1~2 | 1~2 人 | Q4 |
| 3 | 存算分离分离式部署 | 3A RDMA/UB UBOE 场景 | **高** | 引擎挂死导致池化数据丢失是生产级缺陷；独立 Store 进程是存算分离的基础架构；依赖需求 1 传输接口 | ~3 | 3 人 | 18 周 |
| 3 | 存算分离分离式部署 | 3B HCCS（A3）场景 | **中** | HCCS 场景设备亲和性复杂度低于 RDMA，但 A3 是主力推理卡，需支持；依赖 3A 架构验证 | ~1 | 1 人 | 4 周（紧随 3A） |
| 4 | 4 级缓存（L4 共享盘 + SSD 预取） | 4A SSD L4 共享盘穿刺 | **中** | 上游框架已存在（DfsReplicaData+DistributedStorageBackend），仅需昇腾穿刺验证；依赖需求 3 存算分离（独立 Store 进程才有 L4 意义）；Issue #2827 SSD 功能性问题需先修复 | 1.5~2 | 1.5~2 人 | 12 周 |
| 4 | 4 级缓存（L4 共享盘 + SSD 预取） | 4B SSD 预取功能 | **中** | 上游已有 RFC #3417 + draft PR #2646，对接 InferNex cache-indexer/router 天然协同；但依赖 4A L4 层先建好；IO bound 场景收益明确 | 2~3 | 2~3 人 | 10 周（紧随 4A） |
| 5 | Master 去中心化元数据架构 | 5A 去中心化/hierarchical 架构 | **低** | 大集群（万卡级）才触发瓶颈，当前规模短期不痛；上游已有 HttpMetadataServer+NoFSegmentManager+hot_standby 基础设施；涉及 MooncakeStore V3 架构演进，周期最长（24 周）；建议先启动 RFC，实现阶段待需求 4 L4 扩容后协同 | 2.7~3.1 | 3 人 | 24 周 |
| 6 | 社区软件栈加固和优化 | 6A AscendDirectTransport 段错误 | **高** | 生产稳定性阻塞项；InferNex 已贡献 Issue #10532，需确认修复版本。◎ 与 2C HIXL 加固重叠 | 跟踪验证（0.5） | 持续 | 持续 |
| 6 | 社区软件栈加固和优化 | 6B K8s NPU 设备 ID 兼容 | **高** | K8s 部署阻塞项；InferNex 已贡献 Issue #2557，需确认 v0.18.0 是否含修复 | 跟踪验证（0.5） | 持续 | 持续 |
| 6 | 社区软件栈加固和优化 | 6C V1 register_buffer | **高** | PD 分离 decode 获取 KVCache 阻塞项；InferNex 已贡献 Issue #5044 | 跟踪验证（0.3） | 持续 | 持续 |
| 6 | 社区软件栈加固和优化 | 6D P2P HA 恢复优化 | **中** | 影响 PD 分离可用性；PR #3493 推进中。◎ 与 2A/2B HA 加固重叠 | 跟踪+适配（0.5） | 持续 | Q4 |
| 6 | 社区软件栈加固和优化 | 6E SSD 去重修复 | **低** | 依赖 4A L4 建设；PR #3477 已有修复 | 跟踪验证（0.3） | 持续 | Q4 |
| 6 | 社区软件栈加固和优化 | 6F batch io_uring reads | **低** | 依赖 4A L4 建设；PR #3488 性能优化 | 跟踪验证（0.3） | 持续 | Q4 |
| 6 | 社区软件栈加固和优化 | 6G EGM-backed Store pool | **低** | 昇腾无 NVLink，非直接适用；灵衢/RoCE 可参考架构理念；中长期调研 | 评估调研（0.5） | 持续 | 27 Q1~Q2 |
| | **合计** | | | | **~13~16+** | | |

> 优先级判定原则：
> - **高**：阻塞商用部署/生产稳定性/K8s 部署/已有上游基础可快速产出
> - **中**：有明确性能收益但依赖前置需求、或上游框架已有仅需适配
> - **低**：大集群规模才触发/中长期演进/昇腾非直接适用
