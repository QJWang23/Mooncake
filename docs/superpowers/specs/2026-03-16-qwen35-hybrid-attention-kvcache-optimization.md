...
|------|----------------------|-------------------------------|
| **核心机制** | Delta规则 + 门控状态更新 | 固定窗口大小，丢弃旧token |
...
门控参数 β：控制每个时间步的记忆保留比例，使模型能够快速「遗忘」不再相关的历史信息
增量参数 Δ：定义了对现有记忆的更新方式，支持对特定位置的精确修改而非全量覆盖
...
卷积张量（conv1d.weight）：用于局部特征提取，通常采用 1D 卷积核捕获相邻 Token 之间的局部依赖关系，核大小一般设置为 4。这一组件弥补了线性注意力对局部信息建模的不足，增强了模型对短语和搭配的理解能力。
状态矩阵（A_log）：以对数形式存储的状态转移矩阵，在模型加载时通过 -exp(A_log) 转换为实际的转移权重。这种存储方式保证了数值稳定性，避免了在训练过程中可能出现的数据溢出问题。
门控投影（dt_proj）：包含权重（weight）和偏置（bias），用于生成时间步长相关的门控参数，是 Gated DeltaNet 门控机制的核心实现。这一组件使模型能够根据输入内容动态调整记忆更新策略。
跳跃连接（D_proj）：提供从输入到输出的直连通道，增强梯度的反向传播能力，提升模型的训练稳定性。跳跃连接是深度神经网络中的经典技巧，在 SSM 架构中同样发挥着重要作用。

这些组件共同构成了 Gated DeltaNet 的完整计算图。在实际推理过程中，模型通过递归式地更新记忆状态，实现了对任意长度序列的高效处理。与标准 Transformer 需要缓存所有历史 Key-Value 对不同，Gated DeltaNet 只需要维护一个固定大小的记忆状态，这大大降低了长序列推理时的内存占用。
...
enum class KVCacheLayoutType : int32_t {
    ...
    HYBRID = 4,        // Sliding Window Hybrid (通用滑动窗口，用于Qwen2.5模型)
