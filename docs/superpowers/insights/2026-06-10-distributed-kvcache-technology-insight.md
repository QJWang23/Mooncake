...
四个里程碑节点构成了双线规划的核心检查点，每个里程碑都有明确的验收标准和前置依赖，确保项目进展可控。
...
**里程碑之间的逻辑关系：** M1 为 M2 提供上游技术基础（NPU 布局处理器和热缓存优化是 InferNex 性能提升的前提）；M2 为 M3 提供调度能力基础（异构互通需要 KVCache 感知调度的支持）；M3 为 M4 提供异构场景验证基础（智能缓存调度需要在异构环境中经过充分验证）。四个里程碑形成一条从"社区贡献"到"产品交付"的完整链路。
...
**风险 4：openFuyao 社区的国际影响力有限，可能影响上游贡献的可见度。**
...
**风险 5：MemCache 作为 Ascend 原生方案可能与 openFuyao 定位冲突。**
...
**风险 6：HiCache/LMCache 可能向下扩展到编排层。**
...
**风险 7：vLLM-Ascend 对 Ascend 的支持成熟度影响 InferNex 可用性。**
...
**风险 8：Mooncake 社区可能对 NPU 优化的优先级不够高。**
...
以下 Mermaid 甘特图展示了上游贡献线和自研体系线的双线并进规划，以及四个里程碑节点（M1-M4）的时间位置：
...
gantt
    ...
    异构集群 KVCache 互通         :b3, 2027-01-01, 2027-03-31
    智能缓存调度                  :b4, 2027-01-01, 2027-06-30
...
**Q3 2026（双线启动期）：** 上游贡献线和自研体系线同步启动。上游贡献线聚焦 NPU 布局处理器和热缓存优化（对应方向 1 和方向 4），自研体系线启动 KVCache 感知调度和 K8s Operator 开发（对应方向 3）。两条线在 Q3 末交汇于 M1——上游贡献的技术成果（NPU 优化）为自研体系（InferNex 性能提升）提供基础。

**Q4 2026（能力建设期）：** 上游贡献线转向异构传输测试和稀疏注意力适配（对应方向 2 和方向 4），自研体系线完成 KVCache 感知调度和 K8s Operator 的 Beta 版发布。两条线在 Q4 末交汇于 M2——InferNex KVCache 增强版发布，同时 K8s Operator Beta 可供早期用户试用。

**Q1 2027（异构突破期）：** 自研体系线进入异构集群 KVCache 互通和智能缓存调度的开发阶段。上游贡献持续进行（将异构互通的技术产出通用化后贡献到 Mooncake）。Q1 末交汇于 M3——异构互通 PoC 验证通过，证明 Ascend + NVIDIA 混合推理的技术可行性。
...
- **上游贡献线**（Q3-Q4 2026）：4 项具体任务，聚焦 NPU 布局处理器、热缓存优化、异构传输和稀疏注意力适配，目标是在 6 个月内成为 Mooncake 社区 NPU 方向的权威贡献者。
- **自研体系线**（Q3 2026 - Q2 2027）：4 项具体任务，聚焦 KVCache 感知调度、云原生 Operator、异构互通和智能缓存调度，目标是从"组件提供者"升级为"治理平台"。
- **四个里程碑**（M1-M4）提供了季度级别的进度检查点，每个里程碑都有明确的验收标准和前置依赖。
- **八项风险**覆盖了技术、社区、竞争和生态四个维度，每项风险都有具体的缓解策略。
